...
|---|---:|---:|---|
| `SAMformer` | Yes | No package route verified | wrapper exists locally and a first generic real-data smoke now passes; next step is narrow benchmark clear |
...
| `TabPFN-TS` | Yes | repo exists; standalone route verified | local wrappers exist; runtime import is now fixable, but practical use still needs gated model access or a local checkpoint |
| `Prophet` | Yes | Yes, pip package `prophet` | local wrapper exists and a user-local vendor install works; ready for narrow benchmark-clear work |
