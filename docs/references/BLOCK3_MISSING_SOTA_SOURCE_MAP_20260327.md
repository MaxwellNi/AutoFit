...
|---|---|---|---|---|---|---|
| `SAMformer` | Directly aligned with the V740 goal: lightweight single-model forecasting that is simpler than runtime selection | ICML 2024 oral; efficiency-oriented forecaster | `https://arxiv.org/abs/2402.10198` | Official GitHub: `https://github.com/romilbert/samformer` | **Locally integrated wrapper exists** in `src/narrative/block3/models/samformer_model.py`; a freeze-backed generic local smoke passes on `task1_outcome/core_edgar/is_funded/h=14` with non-constant output (`MAE=0.3863`, `fit_seconds=11.6`), and the first narrow benchmark-clear SLURM probe (`5294242`) has now **completed successfully** on the real harness (`MAE=0.3485`, fairness pass true). Still not canonical-benchmark-landed yet | Custom lightweight wrapper |
| `LightGTS` | Strongest current efficiency-first missing baseline; directly relevant to longer-horizon single-model V740 | ICML 2025 official repo claims SOTA on 9 benchmarks in zero-shot and full-shot settings with better efficiency than TSFMs | `https://arxiv.org/abs/2506.06005` | Official GitHub: `https://github.com/decisionintelligence/LightGTS` | Official repo has now been directly audited at HEAD `36ad5bfb4c71ce11bf0372f8e4433c29e1ea5ff5`; it is a script-driven PyTorch project with `finetune.py`, `zero_shot.py`, custom `datautils.py`, and pretrained checkpoints. A Block 3 vendor-path helper exists in `src/narrative/block3/models/optional_runtime.py`, a local wrapper scaffold exists in `src/narrative/block3/models/lightgts_model.py`, the model is locally registered, the first tiny freeze-backed funding smoke is **non-fallback** on `task2_forecast/core_edgar/funding_raised_usd/h=30` after reducing context to `input_size=60`, and the repaired narrow benchmark-clear rerun `5298289` now completes successfully with `MAE=201930.5506` and `fairness_pass=true` | Custom wrapper or vendor import |
| `OLinear` | Cheap but serious decomposition/linear challenger; highly relevant because NBEATS/NHITS-style decomposition is already dominant on Block 3 | NeurIPS 2025 official repo claims strong efficiency and 24-benchmark / 140-task coverage | NeurIPS 2025 poster: `https://neurips.cc/virtual/2025/poster/119234` | Official GitHub: `https://github.com/jackyue1994/OLinear` | Official repo is now directly audited at HEAD `f168e01a3e0e316ad98330b5e77afed1f77b0af5`; a Block 3 vendor-path helper exists in `src/narrative/block3/models/optional_runtime.py`; a local wrapper exists in `src/narrative/block3/models/olinear_model.py`; the model is locally registered; the first tiny funding smoke is non-fallback on `task2_forecast/core_edgar/funding_raised_usd/h=30`; the first tiny investors-count smoke on `h=14` falls back to constant on that small slice; the first audited funding narrow clear `5298296` completed successfully with `MAE=131288.8062` and `fairness_pass=true` | TSLib-style vendor integration |
| `ElasTST` | Highest-value varied-horizon standalone comparator; directly tied to V740’s h1–365 ambition | ProbTS added an official ElasTST branch and varied-horizon scripts in Oct 2024 | OpenReview PDF: `https://openreview.net/pdf/4747a9cde6909a55d4179490101ff0d5f36daf83.pdf` | ProbTS implementation / branch: `https://github.com/microsoft/ProbTS` | Official ProbTS repo is now audited locally; Block 3 vendor-import wrapper exists in `src/narrative/block3/models/elastst_model.py`; first funding smoke is non-fallback on `task2_forecast/core_edgar/funding_raised_usd/h=30`, a shorter-context investors smoke also runs end-to-end, the first funding narrow clear `5298399` completes successfully with `MAE=201925.6990` and `fairness_pass=true`, but the second audited investors-count narrow clear `5298437` returns `fairness_pass=false`. Not yet canonical-landed | ProbTS-style vendor integration or custom wrapper |
| `UniTS` | Strongest unified multi-task/multi-horizon design reference; directly relevant to replacing V739-style runtime selection | NeurIPS 2024 multi-task time-series model | `https://papers.nips.cc/paper_files/paper/2024/file/fe248e22b241ae5a9adf11493c8c12bc-Paper-Conference.pdf` | Official GitHub: `https://github.com/mims-harvard/UniTS` | Official repo is now directly audited at HEAD `0e0281482864017cac8832b2651906ff5375a34e`; a forecasting-only Block 3 wrapper now exists in `src/narrative/block3/models/units_model.py`; the model is locally registered; the first tiny funding smoke on `task2_forecast/core_edgar/funding_raised_usd/h=30` is non-fallback (`MAE=265368.0`); and the first narrow benchmark-clear `5298457` has now completed successfully with `MAE=131725.2212`, `fairness_pass=true`. Still not canonical-benchmark-landed | Script-based vendor integration |
...
| `TabPFN-TS` | Very high information-gain baseline because it is ultra-fast and zero-shot; ideal stress-test against “single-model V740 should be lightweight” | Official repo positions it as zero-shot TS forecasting with TabPFN | NeurIPS 2024 TRL / TSALM workshop line | Official GitHub: `https://github.com/PriorLabs/tabpfn-time-series` | Local generic `TabPFNRegressor/Classifier` wrappers already exist in `src/narrative/block3/models/traditional_ml.py`; the `GLIBCXX_3.4.31` runtime blocker is resolved, HF auth works, official `tabpfn_2_6` checkpoints are cached locally, and the shared `tabpfn 6.3.2` runtime has been superseded by a latest-source vendor install (`tabpfn 7.0.1`) under `~/.cache/block3_optional_pydeps/py312_tabpfn_latest`. The first narrow benchmark-clear SLURM probe (`5294255`) for `TabPFNClassifier` + official 2.6 weights has now **completed successfully** (`MAE=0.1611`, fairness pass true). Two `TabPFNRegressor` narrow clears have now also completed: the funding slice (`5294259`) runs cleanly but shows weak quality, while the investors-count slice (`5294260`) returns a red-flag `fairness_pass=false` result | ML-tabular style custom wrapper |
| `Prophet` | Not the newest, but still high-value as a business / finance sanity baseline that reviewers recognize immediately | Reappears in practical forecasting baselines and is still a useful business-time-series trust anchor | “Forecasting at Scale” | Official GitHub: `https://github.com/facebook/prophet` ; pip package `prophet` | Local wrapper exists in `src/narrative/block3/models/statistical.py`; a user-local vendor install now works, and a generic real-data smoke passes on `task2_forecast/core_only/funding_raised_usd/h=30` (`MAE=5300.86`, non-constant). The first narrow benchmark-clear probe (`5294243`) failed before model execution because `quick` preset does not support `h=30`; the corrected CPU-only resubmission (`5294254`, `v740_prop_std`) has now **completed successfully** (`MAE=1599245.38`, fairness pass true) | Statistical baseline wrapper |
...
|---|---:|---:|---|
| `SAMformer` | Yes | No package route verified | wrapper exists locally, generic smoke passes, and a first narrow benchmark-clear run has now completed successfully |
| `LightGTS` | Yes | No package route verified | official repo structure is audited; local wrapper scaffold exists; import smoke passes; first tiny real-data smoke is non-fallback; repaired narrow benchmark-clear now succeeds, so custom wrapper remains the realistic path and the next honest gate is a wider local-clear matrix |
| `OLinear` | Yes | No package route verified | official repo path is now audited; local wrapper + artifact generation path exist; first funding narrow clear succeeded; count-side evidence remains weak |
...
| `TabPFN-TS` | Yes | repo exists; standalone route verified | local wrappers exist; latest-source vendor runtime (`tabpfn 7.0.1`) is now installed, official `tabpfn_2_6` checkpoints are cached locally, the classifier narrow clear has completed successfully, the funding regressor narrow clear now proves the real harness path works, and the investors-count regressor narrow clear exposes a fairness red flag that must be audited before wider promotion |
| `Prophet` | Yes | Yes, pip package `prophet` | local wrapper exists, a user-local vendor install works, and the corrected CPU-only narrow benchmark-clear has now completed successfully |
