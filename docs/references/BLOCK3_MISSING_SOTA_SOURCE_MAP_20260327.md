...
| `TabPFN-TS` | Yes | repo exists; standalone route verified | local wrappers exist; latest-source vendor runtime (`tabpfn 7.0.1`) is now installed, official `tabpfn_2_6` checkpoints are cached locally, and a first narrow benchmark-clear probe has now completed successfully |
