...
| `TabPFN-TS` | Yes | repo exists; standalone route verified | local wrappers exist, but current env import is blocked by `GLIBCXX_3.4.31`; needs environment fix before benchmarking |
| `Prophet` | Yes | Yes, pip package `prophet` | local wrapper exists, but current env is missing the package; easiest missing sanity baseline once dependency is installed |
