...
| `LightGTS` | Yes | No package route verified | official repo structure is audited; local wrapper scaffold exists; import smoke passes; first tiny real-data smoke is now non-fallback, so custom wrapper remains the realistic path and a narrow benchmark-clear is the next honest gate |
