...
| `ElasTST` | local wrapper now exists; funding smoke is non-fallback; shorter-context investors smoke is also non-fallback; first funding narrow clear completed successfully, but the second investors-count narrow clear now returns `fairness_pass=false`; a more conservative count-side smoke at `input_size=21 / l_patch_size=3_6` remains executable but does not improve enough to justify another immediate harness rerun | keep in local-clear lane; fix count-side fairness before any canonical attempt |
| `UniTS` | official repo now audited at HEAD `0e0281482864017cac8832b2651906ff5375a34e`; a forecasting-only local wrapper exists in `src/narrative/block3/models/units_model.py`; first tiny funding smoke is non-fallback (`MAE=265368.0`); first narrow benchmark-clear `5298457` completed successfully with `MAE=131725.2212`, `fairness_pass=true`; a richer count-side smoke on `investors_count / h=14` is now also non-fallback and strong enough to justify a second harness clear, which is queued as `5298559` | move from docs-only into the same local-clear comparator lane as the other second-wave entrants; next real decision depends on `5298559` |
