...
| --- | --- | --- |
| `SAMformer` | wrapper exists; generic local smoke passes; first audited narrow clear completed (`MAE=0.3485`, fairness pass true); second funding-side tiny smoke is non-fallback (`MAE=265368.0`) and a follow-up narrow clear `5299018` is now queued | use `5299018` to make the real promotion decision instead of guessing from one binary slice |
