...
**If 4090 EDGAR recompute fails (diff_count high):** Data parity issue—4090 raw edgar or edgar store may differ from 3090. Fix: rsync `data/raw/edgar/` and `runs/edgar_feature_store/20260127_133511/` from 3090 to 4090. Temporary workaround: add `--edgar_recompute_required 0` so EDGAR failure does not gate (text/contract checks still run).
