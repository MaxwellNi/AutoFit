#!/bin/bash
#SBATCH --job-name=v740_inv_t2occ_cf
...
#SBATCH --output=/work/projects/eint/logs/phase15/v740_inv_t2occ_cf_%j.out
#SBATCH --error=/work/projects/eint/logs/phase15/v740_inv_t2occ_cf_%j.err
...
JOB_ROOT="$RUN_ROOT/v740_investors_task2_h1_occurrence_probe_cfisch_20260404"
...
run_variant() {
	...
	echo "[v740-inv-t2occ-cf] variant=$variant start $(date -u +'%Y-%m-%dT%H:%M:%SZ')"
	"$INSIDER_PY" scripts/run_v740_shared112_champion_loop.py \
		...
		--skip-existing \
		"$@"
	echo "[v740-inv-t2occ-cf] variant=$variant done $(date -u +'%Y-%m-%dT%H:%M:%SZ')"
}
...
echo "[v740-inv-t2occ-cf] all variants complete $(date -u +'%Y-%m-%dT%H:%M:%SZ')"