...
class ModelMetrics:
    ...
    lane_source_scale_physical_calibration: Optional[bool] = None
    lane_source_scale_reliability: Optional[float] = None
...
class BenchmarkShard:
    ...
    # -----------------------------------------------------------------
    # Text embedding integration (GTE-Qwen2 pre-computed embeddings)
    # -----------------------------------------------------------------
    ...
    def _join_text_embeddings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Reads text_embeddings.parquet (generated by generate_text_embeddings.py)
        ...
        if not emb_path.exists():
            logger.warning(
                ...
                f"Run scripts/generate_text_embeddings.py first. "
                f"Falling back to raw text join (will be dropped by select_dtypes)."
            )
        ...
        if BenchmarkShard._TEXT_EMB_CACHE is None or BenchmarkShard._TEXT_EMB_CACHE_PATH != emb_path:
            ...
            logger.info(
                f"  Text embeddings: {len(BenchmarkShard._TEXT_EMB_CACHE):,} rows, "
                f"{len(emb_cols)} dims"
            )

        emb_df = BenchmarkShard._TEXT_EMB_CACHE

        # Normalize crawled_date_day to string for merge
        df = df.copy()
        df["_cdd_str"] = pd.to_datetime(df["crawled_date_day"]).dt.strftime("%Y-%m-%d")
        emb_df = emb_df.copy()
        emb_df["_cdd_str"] = pd.to_datetime(emb_df["crawled_date_day"]).dt.strftime("%Y-%m-%d")

        emb_cols = [c for c in emb_df.columns if c.startswith("text_emb_")]
        merge_df = emb_df[["entity_id", "_cdd_str"] + emb_cols]

        merged = df.merge(merge_df, on=["entity_id", "_cdd_str"], how="left")
        merged.drop(columns=["_cdd_str"], inplace=True)

        match_rate = merged[emb_cols[0]].notna().mean()
        logger.info(f"  Text embedding match rate: {match_rate:.1%}")
    ...
    def _load_data(self, target: str, horizon: int) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        if ablation_cfg.get("use_text", False):
            try:
                df = self._join_text_embeddings(df)
            except Exception as e:
                logger.warning(f"Could not add text embeddings: {e}")
        ...
        if ablation_cfg.get("use_edgar", False):
            try:
                ...
                # Ensure datetime types for merge_asof — strip timezone AND
                # normalize resolution to ns to avoid "incompatible merge keys"
                # between tz-naive/tz-aware or us/ns on pandas ≥ 3.0
                df["crawled_date_day"] = pd.to_datetime(
                    df["crawled_date_day"], utc=True
                ).dt.tz_convert(None).astype("datetime64[ns]")
            ...
            if hasattr(model, "get_routing_info"):
                ...
                if isinstance(info, dict):
                    ...
                    for _lane_key in (
                        ...
                        "lane_source_scale_physical_calibration",
                        "lane_source_scale_reliability",
    ...
    def run_model(
        ...
        try:
            ...
            result = ModelMetrics(
                ...
                lane_source_scale_physical_calibration=routing_signals.get("lane_source_scale_physical_calibration"),
                lane_source_scale_reliability=routing_signals.get("lane_source_scale_reliability"),
