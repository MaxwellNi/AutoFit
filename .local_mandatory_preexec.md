...
# Last updated: 2026-03-21T12:00 (15515 records, V739@119, 51 npin jobs: gpu 21 + l40s 15 + hopper 15, hp_ac_t1_fu first hopper job RUNNING)
...
- `docs/references/MODEL_REGISTRY.md` (NEW: 152 models, 117 active, 114 in benchmark)
- `docs/references/BLOCK3_CHAMPION_COMPONENT_ANALYSIS.md`

**Directly verified facts on 2026-03-21 01:00** (Full rescan + all corrections):
- Canonical benchmark root remains `runs/benchmarks/block3_phase9_fair/`
- Raw benchmark: **15,374** raw records (+162 in 13h), **137** raw models (116 real + 21 retired AutoFit @1 record each)
- **17 audit-excluded models** (see `scripts/aggregate_block3_results.py` AUDIT_EXCLUDED_MODELS):
  - Finding A: Sundial, TimesFM2, LagLlama, Moirai, MoiraiLarge, Moirai2 (silent context-mean fallback)
  - Finding B: AutoCES, xLSTM, TimeLLM, StemGNN, TimeXer (training crash → global mean fallback)
  - Finding C: TimeMoE, MOMENT (98-99% identical to Timer)
  - Finding G: MICN, MultiPatchFormer, TimeFilter (100% constant predictions)
  - Structural: NegativeBinomialGLM (convergence failure, only 21 records)
- **99 active models** (116 raw - 17 excluded)
- **77 raw models @160/160**, **64 active models @160/160** (77 - 13 excluded@160)
- **15,015 unique records** / ~18,080 max = **83.0%** completion
- 87% task redundancy: ~72 truly unique evaluation cells (not 160)
- Model completion tiers (2026-03-20 verified):
  - @160 complete: 64 active + 13 excluded = 77 raw (↑2: Chronos2+TTM DONE)
  - @159: XGBoost (missing 1: t1/full/is_funded — structural OOM, UNFIXABLE)
  - @157: XGBoostPoisson (missing 3: t1/full/is_funded × h7/h14/h30 — structural OOM, UNFIXABLE)
  - @116: AutoFitV739 (missing s2/e2 — 5 af739 scripts: 4R + 1PD on gpu)
  - @113: 4 TSLib (ETSformer, LightTS, Pyraformer, Reformer) — ALL33 accel RUNNING
  - @109: 4 TSLib (Crossformer, MSGNet, MambaSimple, PAttn) — ALL33 accel RUNNING
  - @61: 21 P15 models (↑7 from 54, ALL33 3-partition accel RUNNING)
  - @61: ModernTCN (↑9, ALL33 accel RUNNING, slow due to 12.5M params)
  - **3-partition parallel strategy active (2026-03-21 01:00):**
    - gpu: 31R + 0PD (ALL33 accel + fix11 + af739 + cos2)
    - l40s: 5R + 10PD (CPU-maxed at 60/64 CPUs, 10PD wait for slots)
    - hopper: 0R + 15PD (blocked by 4 other users' running jobs)
    - cfisch: 0R (cf_p15_t1_ces2 TIMEOUT@2d, covered by ALL33 e2)
    - Total: 61 jobs (36R + 25PD)
- V739 current landed: **116/160** (missing 44 s2/e2 conditions, 5 af739 scripts covering)
- Chronos2: **160/160** ✅ (gpu_fnd_cos2+e2 completed in 7-26 min)
- TTM: **160/160** ✅ (gpu_fnd_cos2+e2 completed in 7-26 min)
- Top-5 by mean rank: PatchTST(4.28), NHITS(4.38), NBEATS(5.01), NBEATSx(5.81), ChronosBolt(7.42)
- Champion: NBEATS — 65/160 wins (40.6%)
- Text embeddings HURT: core_text wins only 11.7% of pairs (55.7% core_only wins)
- EDGAR mixed: wins 34.7% of pairs (target-dependent)
- Full vs core_only: 45.0% vs 42.0% (closest margin)
- Seed stability: avg |delta| = 0.138%, median = 0.000%
- Phase 12 text reruns: 48/48 COMPLETED
- Live queue (2026-03-21 01:00): npin 36R+25PD = 61 total
  - npin gpu: 31R (af739_t3_e2 started!) | l40s: 5R + 10PD | hopper: 0R + 15PD
  - af739: t1_e2(21h), t1_s2/t2_s2/t2_e2(14h each), t3_e2(4h) — all RUNNING
  - l40s 5 RUNNING ~13h: t1_fu, t2_fu, t3_fu (200G,14c), t1_co, t3_co (120G,9c)
  - l40s CPU-maxed: 60/64 CPUs used by 5 jobs → 10PD must wait for slots
  - hopper 15 PENDING: queued behind jgoncalves(4R) + agader/balshaikhd(3PD)
  - cf_p15_t1_ces2 TIMEOUT@2d (covered by gpu_ac_t1_e2 at ~95%)
- Log path: `/work/projects/eint/logs/phase15/` (NOT slurm_logs/)
...
- **ml_tabular OOM** (2026-03-18): Fixed with per-model split script; 6 of 8 ml_tabular now @160
- **Seed2 harness bug** (2026-03-18): Fixed (commit `a9162c2`), cos2 jobs resubmitted
- **GPU migration** (2026-03-19): ALL hopper+l40s pending cancelled, resubmitted on gpu at 7-8 CPUs
- **l40s MaxMemPerCPU=15G constraint** (2026-03-19): See §4d below
- **Admin approved l40s+hopper** (2026-03-20): Julien confirmed high mem/cpu ratio is justified for our workload. l40s(iris-snt) and hopper(besteffort) now active.
- **3-partition parallel** (2026-03-20): 15 l40s + 15 hopper accel scripts submitted alongside 30 gpu jobs. Resume/skip mechanism prevents duplicate work.
...
4. **l40s = ⛔ DO NOT USE** for new benchmark jobs (2026-03-19 结论).
   - DefMemPerCPU=MaxMemPerCPU=**15G** (严格强制！每CPU最大15G，不可覆盖)
   - 我们的 benchmark 最低需要 ~115G (core_only 数据加载基线)
   - 8 cores × 15G = 120G → 勉强够 core_only (115G peak, 5G headroom, 高风险)
   - 2 cores × 15G = 30G → **完全不可行** (admin 建议的 2 cores 方案)
   - 32 CPUs / 4 GPUs = 8 cores/GPU → admin 要求的上限
   - ❗ **结论**: l40s 的 15G/CPU 内存限制使其不适合我们的高内存工作负载。
   - 现有l40s任务跑完后不再提交新任务。全部迁移到gpu分区。
...
5. **hopper = ⛔ DO NOT USE** for new jobs (2026-03-19 admin 合规).
   - Admin 警告：不规则的 CPU 分配 (9/11/12 cores) 导致投诉。
   - 所有 hopper pending 已取消，不再提交。
6. **GPU partition = ONLY ALLOWED** for all new GPU jobs (2026-03-19 policy).
   - DefMemPerCPU=27G, MaxMemPerCPU=UNLIMITED
   - co: --mem=150G → ceil(150*1024/27000)=6 → max(7,6)=7 CPUs ✅
   - ce/ct/e2: --mem=189G → ceil(189*1024/27000)=8 → max(7,8)=8 CPUs ✅  (NOTE: 这里用8因为超过7)
   - fu: --mem=200G → ceil(200*1024/27000)=8 CPUs ✅
   - 28 CPUs / 4 GPUs = 7 CPUs/GPU → 7-8 cores 均可接受

### 4d. DefMemPerCPU CPU分配公式 (血泪教训，2026-03-19)

> **⚠️ 重要**: SLURM 根据内存请求自动提升 CPU 分配！
> `NumCPUs = max(cpus-per-task, ceil(mem_in_MB / DefMemPerCPU))`
> 
> 这意味着:
> - 请求 200G 在 gpu (27G/CPU) 时: max(7, ceil(204800/27000)) = max(7,8) = **8 CPUs**
> - 请求 200G 在 l40s (15G/CPU) 时: max(8, ceil(204800/15000)) = max(8,14) = **14 CPUs** ← admin 投诉原因！
> - 请求 200G 在 hopper (18G/CPU) 时: max(7, ceil(204800/18000)) = max(7,12) = **12 CPUs** ← admin 投诉原因！
>
> **解决方案**: 只使用 gpu 分区 (DefMemPerCPU=27G)，内存请求 ≤200G 保证 ≤8 CPUs。
> 
> | 分区 | DefMemPerCPU | MaxMemPerCPU | CPUs/node | GPUs/node | 推荐 CPUs/GPU |
> |------|-------------|-------------|-----------|-----------|---------------|
> | gpu | 27G | UNLIMITED | 28 | 4 | 7 |
> | l40s | 15G | **15G (hard)** | 32 | 4 | 8 但内存上限120G！ |
> | hopper | 18G | ? | 112 | 4 | 28 但admin 不接受 |
...
17. **af739 s2/e2 全部 TIMEOUT@1d**: 2026-03-19 提交的 5 个 af739 gap-fill 脚本 (t1_s2, t2_s2, t1_e2, t2_e2, t3_e2) 全部在 1d 内超时。AutoFit V739 每个条件 (8 candidates × 3 targets × 4 horizons) 计算量大，1d 不够。2026-03-20 把所有脚本改为 2d 并重新提交 (jobs 5267972-5267975)。af739_t1_e2 正在运行中（job 5266705，无法 scontrol 延时）。**教训：AutoFit V739 所有条件必须用 2d 时限。**
18. **冗余 l40s RUNNING 曾浪费资源**: l40_cos2_t2 在 l40s 上分配了 14 CPUs (因为 200G / 15G DefMemPerCPU = 14)，但实际只用 1 CPU + 88G 内存。同时 gpu_cos2_t2 已经在 gpu 上运行。HPC admin 投诉了。**但 admin 随后确认**：如果确实需要 200G，这不算浪费（"as you have a rationale for this bigger cpu/gpu ratio this is not waste"）。**教训：提交前检查是否有冗余任务；l40s 高 CPU 分配现已获 admin 批准，但需有合理理由。**
19. **时间戳必须来自 `date` 命令**: 永远不要猜测或使用未来的时间。每次写报告前先 `date +%H:%M` 确认当前时间。
20. **l40s CPU 分配已获 admin 批准**: l40s 的 MaxMemPerCPU=15G 是硬限制。SLURM 公式: NumCPUs = max(cpus-per-task, ceil(mem/DefMemPerCPU))。200G → 14 CPUs，120G(co) → 8 CPUs。admin Julien 2026-03-20 确认我们的高内存需求是合理理由，l40s 可以正常使用。**优化策略：co/s2 用 120G(8c) 而非 200G(14c) 以减少CPU占用。ce/ct/e2 用 200G(14c)，fu 用 200G(14c)。l40s iris-snt QOS。**
21. **AutoFit V739 增量恢复有效**: af739 脚本 TIMEOUT 后重新提交，harness 会跳过已完成的条件（resume/skip）。V739 在反复超时中从 113→116 条件。说明 benchmark harness 的增量写入机制正常工作。
22. **gpu_fnd 任务很快完成**: Chronos2+TTM 的 5 个 cos2/e2 gap-fill 脚本只需 7-26 分钟就完成了。Foundation model 的 seed2/e2 条件计算量极小。如果之后有类似 foundation 模型的 gap-fill，不需要分配很长时间。
23. **ALL33 accel 脚本进度不均**: 跑了 20+ 小时的 gpu_ac 脚本，有的处理了 23 模型（ce/ct 第二轮），有的只处理了 4-5 个（co/fu 新模型慢）。DeformableTST 训练特别慢 (CUDA OOM + 常量预测问题)，是瓶颈。进度取决于模型训练速度不是条件数量。
24. **三分区并行策略**: admin 批准后，gpu+l40s+hopper 三个分区同时运行相同的 ALL33 脚本。benchmark harness 的 resume/skip 机制确保不会重复计算。同一模型+条件可能在两个分区同时开始训练，但这只浪费一次训练时间，不影响结果正确性。**这是最大化吞吐量的最佳策略。**
25. **分区 QOS 和 CPU 实际分配表**: 
    - gpu: qos=normal, DefMemPerCPU=27G, MaxMemPerCPU=27G. 150G→6c, 189G→7c(实际8c因cpus-per-task), 200G→8c
    - l40s: qos=iris-snt, DefMemPerCPU=15G, MaxMemPerCPU=15G. 120G→8c, 200G→14c. L40S GPU=48GB VRAM
    - hopper: qos=besteffort(可抢占!), DefMemPerCPU=18G, MaxMemPerCPU=18G. 150G→9c, 189G→11c, 200G→12c. H100 GPU=80GB VRAM
    - bigmem: no GPU, 3TB RAM, 112 CPUs
    - 我们的账户 **没有 iris-hopper QOS**，只能用 besteffort（可被抢占+重排队）。
26. **l40s/hopper GPU VRAM 优势**: V100=32GB, L40S=48GB, H100=80GB。DeformableTST 在 V100 上 CUDA OOM（15.77GiB occupied），但在 L40S/H100 上应该可以正常训练。ModernTCN(12.5M参数) 在更快的 GPU 上也训练更快。**瓶颈模型应优先在 l40s/hopper 上运行。**
27. **重复提交必须立即取消**: 本次发现 4 个重复 af739 PENDING (5267972-75 与 5267968-71 重复) 和 15 个重复 l40s (5268080-94 与 5268065-79 重复)。**提交前务必 `squeue -u npin -n <job_name>` 检查是否已有同名任务。** 发现重复后立即 scancel。
28. **hopper besteffort QOS 意味着可被抢占**: 我们的 hopper 任务优先级低于 iris-hopper QOS 的任务（其他用户在用 iris-hopper）。被抢占后 --requeue 自动重新排队，harness resume/skip 机制保证进度不丢失。但频繁抢占 = 频繁重新加载 dataset (~10min)。
29. **SLURM 日志输出在 .err 而非 .out**: benchmark harness 的进度信息（Running/Skipping/MAE= 等）全部输出到 stderr (.err 文件)。.out 文件只有 3 行 header（job name, timestamp, GPU name）。**解析进度必须读 .err 文件。** .err 文件可能非常大（1.7M+ 行），解析用 awk 单遍而不是多次 grep。
30. **l40s CPU 是真正瓶颈**: 5 个 running job 就用了 60/64 CPU (14c×3 + 9c×2 = 60)。**l40s 上同时最多只能跑 4-5 个任务。** 10 个 PENDING 必须等前面的完成。优化：co 用 120G(8c) 而非 200G(14c) 节省 CPU 是正确策略。
31. **不需要额外提交任何脚本**: 所有 6 种 ablation × 3 tasks 都已被现有 running/pending 作业覆盖。任何 TIMEOUT 的作业都会被其他分区的后续作业通过 skip 机制接力完成。cfisch 的 ces2 全部失败，但被 ALL33 e2 脚本覆盖（~95%完成）。
32. **MICN 在 ALL33 脚本中产生常量预测**: gpu_ac_t1_co 日志显示 MICN 训练正常但 predict failed → 100% constant prediction → fairness_pass=False。MICN 已在 AUDIT_EXCLUDED_MODELS 中。不影响结果但浪费了训练时间（~6min）。
...
## §39. Seed2 Ablation Name Bug 发现与修复 (2026-03-18, commit a9162c2)

### 39.1 关键 Bug

Harness CLI `--ablation` 只接受 `core_only, core_text, core_edgar, full` 四个名称。
`core_only_seed2` 和 `core_edgar_seed2` 从未被加入 ABLATION_NAMES — **导致所有 seed2/e2 脚本在0-10秒内失败**。

受影响脚本（共20+作业, 全部 EXIT_CODE=2）:
- AF739 s2/e2: 5260365-5260369
- ALL33 accel e2: 5257985-5257993
- P15 cos2: 5256682-5256690
- cfisch ces2: 5256699-5256711

### 39.2 修复方案

- `scripts/run_block3_benchmark_shard.py` L118: ABLATION_NAMES 增加 `core_only_seed2`, `core_edgar_seed2`
- L402+: 新增 `_SEED2_BASE` 映射: `core_only_seed2→core_only`, `core_edgar_seed2→core_edgar`
- `_get_ablation_config()`: 先查 `_SEED2_BASE` 再查 base ablation config
- Commit: `a9162c2`

### 39.3 Seed2 历史机制

Phase 9 原始 seed2 数据通过"2-seed replication"创建:
- core_text 重命名为 core_only_seed2
- full 重命名为 core_edgar_seed2
- metrics.json 中 ablation 字段手动编辑

现在 harness 原生支持这些名称, 映射到对应的 base feature config。

### 39.4 重提交的作业

**npin (修复后重提交):**
- AF739 s2/e2: 5262777-5262781 (5 gpu jobs)
- P15 cos2: 5262782-5262784 (3 gpu jobs)
- ALL33 accel e2: 5262801-5262809 (3 gpu + 3 hp + 3 l40 = 9 jobs)
- ALL33 cos2 (新脚本): 5262817-5262822 (2 gpu + 2 hp + 2 l40 = 6 jobs)
- ml_t1_fu_bm: 5262824 (bigmem, 640G)

**cfisch (修复后重提交):**
- cf_p15_ces2: 5262788-5262790 (3 gpu jobs)

### 39.5 重复作业清理

发现上一轮提交和本轮均提交了同样的脚本, 产生重复 job。已 scancel 24 npin 重复 + 3 cfisch 重复。

### 39.6 CATS/FITS/KANAD/WPMixer ct/fu 已完成 ✅

gf2_old4 六个作业 (5260359-5260364) 全部 COMPLETED:
- t1_ct (2h17m), t1_fu (2h45m), t2_ct (1h18m), t2_fu (1h29m), t3_ct (1h15m), t3_fu (1h28m)
- 四个模型从 @128 → @160 (complete!) ✅

### 39.7 当前 Benchmark

- **14,352** raw records, **114** models, **69** complete@160
- @157: 10 ml_tabular (t1_fu 缺口, bigmem 运行中)
- @112: AutoFitV739 (s2/e2 已重提交)
- @111: ETSformer/LightTS/Pyraformer/Reformer (ALL33 accel 覆盖)
- @105: Crossformer/MSGNet/MambaSimple/MultiPatchFormer/PAttn/TimeFilter (ALL33 accel 覆盖)
- @29: ModernTCN (ALL33 accel 覆盖)
- @23: 22 P15 new models (运行中 + accel 覆盖)
- @21: NegativeBinomialGLM (结构性失败, 排除)

### 39.8 计算资源 (清理后)

- npin: 16 RUNNING + 48 PD = 64 作业 (gpu/l40s/hopper/bigmem)
- cfisch: 8 RUNNING + 13 PD = 21 作业 (gpu/l40s/hopper)
- 总计: 85 作业, ~26 GPUs 同时计算
- GPU 集群: 24 nodes × 4 GPUs = 96 total, ~85% 占用率
- L40S: 2 nodes 满载
- Hopper: 1 node (besteffort, 部分占用)

### 39.9 新教训

67. **`core_only_seed2`/`core_edgar_seed2` 不是原生 ablation 名称** — 直到 commit `a9162c2` harness 才支持。所有使用这些名称的脚本此前全部静默失败 (exit 2, 0-10秒)。
68. **seed2 失败模式极其隐蔽** — 失败在 argparse 层面, 无任何模型训练日志。sacct 显示 FAILED + 0秒运行时间是唯一线索。
69. **ml_tabular full ablation 需要 640G+ 内存** — gpu (756G) 不够因为其他作业共享, bigmem (3TB) 足够。
70. **检查重复作业** — 多轮修复/重提交容易产生重复, 应在每轮提交前先 `squeue | sort` 检查。
71. **cos2 (core_only_seed2) 曾被完全遗漏** — accel 脚本只有 e2, 没有 cos2。必须单独创建。

## §40 冠军模型深度分析更新 (2026-03-18)

### 40.1 冠军分析刷新

基于 14,352 records / 69 complete@160 模型完成全面冠军分析更新:
- `docs/references/BLOCK3_CHAMPION_COMPONENT_ANALYSIS.md` 全面重写 §0-§6
- 从 8,660→14,352 records, 104→160 evaluation cells
- 关键发现:
  1. **Text embeddings HURT**: core_text 只赢 11.7% pairs, core_only 赢 55.7%
  2. **DeepNPTS 是 task1 专属**: 16 wins 全部在 task1_outcome, task2/3 零胜
  3. **PatchTST 最稳定**: mean rank 4.28 (第一), 但只有 4 wins (generalist vs specialist)
  4. **NBEATS 依然统治**: 65/160 wins (40.6%)
  5. **Seed2 高度稳定**: avg |delta| = 0.12%, median = 0.00%

### 40.2 作业进度 (2026-03-18 13:15 CET)

当前运行作业进度 (Training complete 计数):
- p15_new_t2_ce (5253906): 38 completions, current=SRSNet ~38h remain
- p15_new_t3_co (5253907): 86 completions, current=TimeRecipe ~10h remain
- p15_new_t3_ce (5253908): 35 completions ~40h remain
- p15_fix11_t1_co (5256354): 37 completions, current=ModernTCN ~10h remain
- p15_fix11_t1_ce (5256355): 37 completions, current=ModernTCN ~10h remain
- p15_fix11_t2_co (5256356): 44 completions, current=DeformableTST ~24h remain
- l40_fix11_t2_co (5256608): 50 completions ~2h remain (nearly done!)
- gf_ts_t1_co (5256391): 8 completions ~4h remain
- gf_ts_t1_ce (5256392): 2 completions ~12h remain
- gf_t2_co (5256685): 2 completions ~20h remain
- gf_t2_ce (5256686): 2 completions ~20h remain

### 40.3 新教训

72. **Job logs go to stderr**: 所有Python logging输出去.err文件, .out只有script header. 检查进度要看.err不是.out.
73. **P15模型从23→24 records**: 部分新结果已悄悄landed (可能来自timeout后的resume).

---

## §41. TIMEOUT 浪潮与 s2 缺口补填 (2026-03-21T12:00)

### 41.1 TIMEOUT 浪潮

2026-03-21 ~06:00 CET，首波 ALL33 accel 脚本集体 TIMEOUT@2d:
- gpu_ac_t{1,2,3}_e2: done=55-58, skip=25-26 each (ALL TIMEOUT)
- gpu_cos2_t{1,2}: done=73, skip=65 each (ALL TIMEOUT)
- p15_t{1,2,3}_cos2: done=92, skip=40 each (ALL TIMEOUT)
- p15_new_t{1,2}_co, p15_new_t2_co: TIMEOUTs
- cf_p15_t1_ces2: TIMEOUT
- af739_t1_e2 (5266705): TIMEOUT@1d (旧1d时限，非2d)

所有 TIMEOUT 作业均已写入部分合法结果。benchmark rescan 确认 15515 records (↑303 from 15212)。

### 41.2 关键缺口

**s2 (core_only_seed2) 缺口**: gpu_ac scripts 覆盖 {co,ce,ct,e2,fu} 五种 ablation，但**不包含 s2**。s2 只由单独的 cos2 脚本覆盖，而这些脚本全部 TIMEOUT 且无替代进入队列。
- 发现: 27 models 缺 t2 s2 (192 conditions), 即 ALL33 + 4 old models
- 修复: 重提交 gpu_cos2_t2 → Job 5268391

**af739_t1_e2 缺口**: 旧脚本有 --time=1-00:00:00 (1天)，已处理 done=777 skip=24 last=AutoFitV731，非常接近完成。
- 修复: 重提交 af739_t1_e2 (已有2d时限) → Job 5268390

### 41.3 当前资源

- 51 npin jobs: 25 RUNNING + 26 PENDING (gpu 21 + l40s 15 + hopper 15)
- hopper 首个实际运行: hp_ac_t1_fu on iris-197 (共享 3 jgoncalves jobs)
- Chronos2 + TTM: s2 完成 @20/20 ✅ → 总计 @160 ✅

### 41.4 Benchmark 快照

- 15,515 raw records, 137 models, 99 active, 64@160
- Per-ablation: co=2803, s2=2128, ce=2780, e2=2600, ct=2617, fu=2587
- V739: 119/160 (co=28,ce=28,ct=28,fu=28,s2=3,e2=4)
- P15 ALL at 64/160 (↑3 from 61)

### 41.5 新教训

74. **gpu_ac 不覆盖 s2** — accel 脚本的 ablation 参数只有 co/ce/ct/e2/fu。cos2 (core_only_seed2) 需要独立的 cos2 脚本。TIMEOUT 后如果不检查 cos2 进度，s2 缺口会无声扩大。
75. **ALL33 2d TIMEOUT 是预期行为** — 33个模型在单个 V100 上跑一个 ablation 的一个 task，2d 不够。3-partition 策略下，l40s/hopper 接力。每轮 TIMEOUT 写入部分结果，下轮自动跳过 (done_combos)。多轮迭代最终收敛。
76. **af739 旧脚本时限不一致** — gapfill2 目录的 af739 脚本有 2d 时限，但 af739_t1_e2 原始提交用的是 1d。处理 777 records 后 TIMEOUT。重提交前必须确认 --time 参数。
77. **hopper besteffort 首次确认可行** — hp_ac_t1_fu 实际启动在 iris-197，与其他用户共享。besteffort 排队延迟后最终获得资源。l40s/hopper 分区的所有 PENDING 作业不浪费——队列位置保留。

---

## §42. Finding H: 8 个 P15 模型 100% 常数预测 (2026-03-22T02:30)

### 42.1 发现 (Finding H)

全面扫描所有 16 个 RUNNING 作业的 stderr 日志，发现 CONSTANT-PREDICTION 警告集中在 8 个 P15 模型：
- CFPT: 130 次常数预测警告
- DeformableTST: 127 次 (CUDA OOM on V100/L40S, dimension mismatch on H100)
- MICN: 105 次 (已在 Finding G 中排除)
- PathFormer: 92 次
- SparseTSF: 91 次
- SEMPO: 91 次
- TimePerceiver: 85 次
...
Python fairness 检查确认: 这 8 个模型 **0% fairness pass** (所有已测试条件均为常数预测)。

### 42.2 DeformableTST 详细失败模式

DeformableTST 是最严重的瓶颈——每个条件耗时 26 分钟到 2 小时，最终**总是**产生常数预测:
- **V100 (gpu)**: CUDA OOM ("Tried to allocate 318.00 MiB, GPU has 15.77 GiB total, 20.69 MiB free")
- **H100 (hopper)**: tensor dimension mismatch ("size of tensor a (60) must match size of tensor b (169)")
- **L40S**: CUDA OOM ("Tried to allocate 634.00 MiB, GPU has 44.42 GiB total, 202.12 MiB free")
- Training 耗时 1578-7902 秒/条件，全部产生 `best_val_loss=0.000000`，输出常数 `prediction=2408647.5000`

### 42.3 采取措施

1. 7 个新模型加入 `AUDIT_EXCLUDED_MODELS` (Finding H): CFPT, DeformableTST, PathFormer, SEMPO, SparseTSF, TimeBridge, TimePerceiver
2. 总排除模型数: 17 → 24, 活跃模型: 99 → 92
3. 取消 22 个旧 v1 l40s/hopper PENDING 作业 (含 10 个坏模型)
4. 创建 accel_v2 脚本: 23 个有效模型 (移除 10 个坏模型/已排除模型), ~30% 更快
5. 提交 51 个新优化作业: 17 GPU + 17 L40S + 17 Hopper
6. 捕获并取消 17 个意外重复的 GPU 提交

### 42.4 accel_v2 脚本

位置: `.slurm_scripts/phase15/accel_v2/`
命名: `{prefix}_ac_t{task}_{ablation}.sh`, prefix = g2(gpu), l2(l40s), h2(hopper)
23 个模型: CARD,Crossformer,DUET,ETSformer,FiLM,FilterTS,FreTS,Fredformer,LightTS,MSGNet,MambaSimple,ModernTCN,NonstationaryTransformer,PDF,PIR,PAttn,Pyraformer,Reformer,SCINet,SRSNet,SegRNN,TimeRecipe,xPatch
移除 10 个: CFPT,DeformableTST,MICN,MultiPatchFormer,PathFormer,SEMPO,SparseTSF,TimeBridge,TimeFilter,TimePerceiver

内存配置:
- gpu: co/s2=150G/7c, ce/e2/ct=189G/8c, fu=200G/8c
- l40s: co/s2=120G/8c, ce/e2/ct/fu=200G/14c
- hopper: co/s2=150G/9c, ce/e2=189G/11c, ct=189G/11c, fu=200G/12c

### 42.5 新教训

78. **P15 常数预测模型浪费 ~30% 计算时间** — DeformableTST 等模型虽然训练失败 (CUDA OOM / shape mismatch)，但 harness 的 fallback 机制仍然写入常数预测结果。每个条件耗时 15min-2h，乘以每个 ALL33 脚本中的 8 个坏模型 = 巨量浪费。必须从脚本中移除已知坏模型。
79. **fairness_pass 检查是识别坏模型的最可靠方法** — 比查看日志更可靠。直接对 metrics.json 做 `prediction_std > 0` 或 `fairness_pass == True` 过滤，0% pass rate 即为确认坏模型。
80. **终端 sbatch 循环可能静默双重提交** — 第一次 for 循环执行但终端无可见输出，第二次执行也启动。必须在提交前后都用 `squeue -u npin | wc -l` 验证作业数。本次发现并取消了 17 个重复 GPU 作业。
81. **e2 (core_edgar_seed2) 是最关键的覆盖缺口** — ETSformer/LightTS/Pyraformer/Reformer 有 0/28 e2 条件。P15 模型仅 6/28 e2。accel_v2 脚本覆盖 e2 ablation，这是第一优先级。
82. **3-partition 策略下 skip/resume 安全防止重复** — gpu/l40s/hopper 同时运行相同条件，harness 的 done_combos 检查保证每个条件只执行一次。先完成的分区写入结果，后到的分区自动跳过。
