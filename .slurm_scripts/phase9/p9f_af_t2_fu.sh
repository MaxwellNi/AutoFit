#!/usr/bin/env bash
#SBATCH --job-name=p9f_af_t2_fu
#SBATCH --partition=bigmem
#SBATCH --qos=normal
#SBATCH --time=2-00:00:00
#SBATCH --mem=640G
#SBATCH --cpus-per-task=28
#SBATCH --output=/home/users/npin/repo_root/slurm_logs/phase9/p9f_af_t2_fu_%j.out
#SBATCH --error=/home/users/npin/repo_root/slurm_logs/phase9/p9f_af_t2_fu_%j.err
#SBATCH --export=ALL
...
echo ">>> V735: task2_forecast | full | $(date -Iseconds)"
"${INSIDER_PY}" scripts/run_block3_benchmark_shard.py \
    --task task2_forecast --category autofit --ablation full \
    --preset full --output-dir runs/benchmarks/block3_phase9_fair/task2_forecast/autofit/full --seed 42 \
    --no-verify-first --models AutoFitV735
echo ">>> V735 done: $(date -Iseconds)"

# V734 (3-model ensemble with fixed stack_k)
echo ">>> V734: task2_forecast | full | $(date -Iseconds)"
"${INSIDER_PY}" scripts/run_block3_benchmark_shard.py \
    --task task2_forecast --category autofit --ablation full \
    --preset full --output-dir runs/benchmarks/block3_phase9_fair/task2_forecast/autofit/full --seed 42 \
...
echo ">>> V736: task2_forecast | full | $(date -Iseconds)"
"${INSIDER_PY}" scripts/run_block3_benchmark_shard.py \
    --task task2_forecast --category autofit --ablation full \
    --preset full --output-dir runs/benchmarks/block3_phase9_fair/task2_forecast/autofit/full --seed 42 \
