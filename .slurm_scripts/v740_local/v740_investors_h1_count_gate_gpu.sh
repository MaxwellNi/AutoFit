...
#SBATCH --qos=normal
#SBATCH --output=/work/projects/eint/logs/phase15/v740_inv_gate_%j.out
#SBATCH --error=/work/projects/eint/logs/phase15/v740_inv_gate_%j.err
...
JOB_ROOT="$RUN_ROOT/v740_investors_h1_count_gate_20260404"
...
run_variant() {
	...
	echo "[v740-inv-gate] variant=$variant start $(date -u +'%Y-%m-%dT%H:%M:%SZ')"
	"$INSIDER_PY" scripts/run_v740_shared112_champion_loop.py \
		...
		--skip-existing \
		"$@"
	echo "[v740-inv-gate] variant=$variant done $(date -u +'%Y-%m-%dT%H:%M:%SZ')"
}
...
echo "[v740-inv-gate] all variants complete $(date -u +'%Y-%m-%dT%H:%M:%SZ')"