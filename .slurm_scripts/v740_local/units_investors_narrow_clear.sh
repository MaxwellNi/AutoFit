#!/bin/bash
#SBATCH --job-name=v740_units_inv_clr
#SBATCH --account=npin
...
/mnt/aiongpfs/projects/eint/envs/.micromamba/envs/insider/bin/python3 scripts/run_block3_benchmark_shard.py \
  ...
  --models UniTS \
  --model-kwargs-json '{"UniTS":{"input_size":30,"patch_len":5,"stride":5,"d_model":64,"prompt_num":4,"e_layers":2,"n_heads":4,"max_epochs":5,"batch_size":16,"max_covariates":8,"max_entities":1000,"max_windows":12000}}' \
  --ablation core_edgar \
  ...
  --max-entities 16 \
  --max-rows 1600 \
  --output-dir runs/benchmarks/block3_phase9_localclear_20260330/units_investors_h14_ctx30
