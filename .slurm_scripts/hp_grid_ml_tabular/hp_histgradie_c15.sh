#!/usr/bin/env bash
#SBATCH --job-name=hp_histgradie_c15
...
#SBATCH --gres=gpu:1
#SBATCH --output=/work/projects/eint/logs/hp_grid_ml_tabular/hp_histgradie_c15_%j.out
#SBATCH --error=/work/projects/eint/logs/hp_grid_ml_tabular/hp_histgradie_c15_%j.err
#SBATCH --export=ALL
#SBATCH --requeue
...
echo "============================================================"
echo "HP Grid Search | Job ${SLURM_JOB_ID} | Config #15"
echo "$(date -Iseconds) | $(hostname)"
echo "Model: HistGradientBoosting | Category: ml_tabular"
echo "Task: task1_outcome | Ablation: core_only"
echo "HP Config: {\"HistGradientBoosting\": {\"l2_regularization\": 1.0, \"learning_rate\": 0.05, \"max_depth\": 3, \"max_iter\": 100, \""
echo "============================================================"
...
"${INSIDER_PY}" scripts/run_block3_benchmark_shard.py \
    ...
    --ablation core_only \
    --preset full \
    --output-dir runs/benchmarks/block3_20260203_225620_phase7/hp_grid/ml_tabular/HistGradientBoosting/config_15 \
    --seed 42 \
    --no-verify-first \
    --models HistGradientBoosting \
    --model-kwargs-json '{"HistGradientBoosting": {"l2_regularization": 1.0, "learning_rate": 0.05, "max_depth": 3, "max_iter": 100, "min_samples_leaf": 5}}' &
